...
def load_language_file():
    ...
    choice = simpledialog.askstring("Language Selection", "Select language:\n1. Polski\n2. English\n3. Português")
...
def display_results(results, tree):
    ...
    if results:
        for result in results:
            tree.insert('', 'end', values=(result['id'], result['name'], result['attributes']))
    else:
...
def edit_item_callback(tree, xml_tree, xml_root):
    ...
    tk.Button(edit_window, text="Save", command=save_changes).grid(row=row, column=0, columnspan=2, pady=10)
...
def create_gui():
    ...
    result_tree = ttk.Treeview(root, columns=("ID", "Name", "Attributes"), show="headings")
    ...
    search_button = tk.Button(frame, text=language["search"], command=lambda: search_callback(entry, xml_root, result_tree))
    search_button.grid(row=0, column=2, padx=5, pady=5)

    edit_button = tk.Button(root, text=language["edit_item"], command=lambda: edit_item_callback(result_tree, xml_tree, xml_root))
    edit_button.pack(pady=5)

    save_button = tk.Button(root, text=language["save_file"], command=lambda: save_file_callback(xml_tree))
    save_button.pack(pady=5)

    file_button = tk.Button(root, text=language["load_file"], command=load_file_and_update_globals)
